...
public class IslandAgent extends Agent {
    ...
    public void mutation(){
        ...
        if(random.nextDouble()>GAUtils.MUTATION_PROP){
            int alphIndex = random.nextInt(GAUtils.ALPHABET.length());
            int chromIndex=random.nextInt(GAUtils.CHROMOSOME_SIZE);
            individual1.getChromosome()[chromIndex]=GAUtils.ALPHABET.charAt(alphIndex);

        }
        if(random.nextDouble()>GAUtils.MUTATION_PROP){
            int alphIndex = random.nextInt(GAUtils.ALPHABET.length());
            int chromIndex=random.nextInt(GAUtils.CHROMOSOME_SIZE);
            individual2.getChromosome()[chromIndex]=GAUtils.ALPHABET.charAt(alphIndex);
        }
